...
static uint8  USBMIDI_1_started = 0u;


/*******************************************************************************
* Function Name: USBMIDI_1_CyBtldrCommStart
****************************************************************************//**
*
...
void USBMIDI_1_CyBtldrCommStart(void) 
{
    ...
    USBMIDI_1_Start(0u, USBMIDI_1_DWR_POWER_OPERATION);

    /* USB component started, the correct enumeration will be checked in the first Read operation. */
    USBMIDI_1_started = 1u;
}


/*******************************************************************************
* Function Name: USBMIDI_1_CyBtldrCommStop.
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: USBMIDI_1_CyBtldrCommReset.
****************************************************************************//**
*
...
{
    USBMIDI_1_EnableOutEP(USBMIDI_1_BTLDR_OUT_EP); 
}


/*******************************************************************************
* Function Name: USBMIDI_1_CyBtldrCommWrite.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    USBMIDI_1_LoadInEP(USBMIDI_1_BTLDR_IN_EP, pData, USBMIDI_1_BTLDR_SIZEOF_READ_BUFFER);

    /* Wait unitl host reads data from IN endpoint. */
    while ((USBMIDI_1_GetEPState(USBMIDI_1_BTLDR_IN_EP) == USBMIDI_1_IN_BUFFER_FULL) &&
           (0u != timeoutMs))
    {
        CyDelay(USBMIDI_1_BTLDR_WAIT_1_MS);
        timeoutMs--;
    }

    if (USBMIDI_1_GetEPState(USBMIDI_1_BTLDR_IN_EP) == USBMIDI_1_IN_BUFFER_FULL)
    {
...
/*******************************************************************************
* Function Name: USBMIDI_1_CyBtldrCommRead.
****************************************************************************//**
*
                                                            ...
                                                            
{
    ...
    if (size > USBMIDI_1_BTLDR_SIZEOF_WRITE_BUFFER)
    {
        size = USBMIDI_1_BTLDR_SIZEOF_WRITE_BUFFER;
    }

    /* Wait for enumeration first time. */
    if (0u != USBMIDI_1_started)
    {
        /* Wait for device enumeration. */
        while ((0u == USBMIDI_1_GetConfiguration()) && (0u != timeoutMs))
        {
            CyDelay(USBMIDI_1_BTLDR_WAIT_1_MS);
            timeoutMs--;
        }
        ...
        {
            (void) USBMIDI_1_IsConfigurationChanged();  /* Clear configuration changes state status. */
            USBMIDI_1_CyBtldrCommReset();
            
            USBMIDI_1_started = 0u;
        }
    }
    else /* Check for configuration changes, has been done by Host. */
    {
        if (0u != USBMIDI_1_IsConfigurationChanged()) /* Host could send double SET_INTERFACE request or RESET. */
        {
            if (0u != USBMIDI_1_GetConfiguration())   /* Init OUT endpoints when device reconfigured. */
            {
                USBMIDI_1_CyBtldrCommReset();
            }
        }
    ...
    while ((USBMIDI_1_GetEPState(USBMIDI_1_BTLDR_OUT_EP) != USBMIDI_1_OUT_BUFFER_FULL) && \
           (0u != timeoutMs))
    {
        CyDelay(USBMIDI_1_BTLDR_WAIT_1_MS);
        timeoutMs--;
    ...
    if (USBMIDI_1_GetEPState(USBMIDI_1_BTLDR_OUT_EP) == USBMIDI_1_OUT_BUFFER_FULL)
    {
        *count = USBMIDI_1_ReadOutEP(USBMIDI_1_BTLDR_OUT_EP, pData, size);
        retCode = CYRET_SUCCESS;
    }
    else
    {
