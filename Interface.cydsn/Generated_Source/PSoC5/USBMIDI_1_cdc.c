 ...
 * USBMIDI_1_GetDTERate(), USBMIDI_1_GetCharFormat(),
 * USBMIDI_1_GetParityType(), and USBMIDI_1_GetDataBits() APIs.
...
volatile uint8 USBMIDI_1_linesCoding[USBMIDI_1_MAX_MULTI_COM_NUM][USBMIDI_1_LINE_CODING_SIZE] =
{
    /*COM Port 1*/
    {
...
/**Used as a flag for the USBMIDI_1_IsLineChanged() API, to inform it that the
 ...
 * support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBMIDI_1_linesChanged[USBMIDI_1_MAX_MULTI_COM_NUM];
 ...
 * USBMIDI_1_GetLineControl() API. It is an array of 2 elements for COM 
 * port 1 and COM port 2 for MultiCOM port support. In case of 1 COM port, data 
 * is in 0 element.*/
volatile uint16 USBMIDI_1_linesControlBitmap[USBMIDI_1_MAX_MULTI_COM_NUM];
/** Contains the 16-bit serial state value that was sent using the 
 * \ref USBMIDI_1_SendSerialState() API. . It is an array of 2 elements 
 * for COM port 1 and COM port 2 for MultiCOM port support. In case of 1 COM 
 * port, data is in 0 element.*/
volatile uint16 USBMIDI_1_serialStateBitmap[USBMIDI_1_MAX_MULTI_COM_NUM];
...
volatile uint8  USBMIDI_1_cdcDataInEp[USBMIDI_1_MAX_MULTI_COM_NUM];
 ...
 * port 2 for MultiCOM port support. In case of 1 COM port, data is in 0 element.*/
volatile uint8  USBMIDI_1_cdcDataOutEp[USBMIDI_1_MAX_MULTI_COM_NUM];
...
volatile uint8  USBMIDI_1_cdcCommInInterruptEp[USBMIDI_1_MAX_MULTI_COM_NUM];

/*PRIVATE*/

#define USBMIDI_1_CDC_IN_EP      (0u)
#define USBMIDI_1_CDC_OUT_EP     (1u)
#define USBMIDI_1_CDC_NOTE_EP    (2u)

#define USBMIDI_1_CDC_EP_MASK    (0x01u)

#define USBMIDI_1_GET_EP_COM_NUM(cdcComNums, epType) (((cdcComNums) >> (epType)) & USBMIDI_1_CDC_EP_MASK)
...
***************************************/
    ...
    static uint16 USBMIDI_1_StrLen(const char8 string[]) ;
    static t_USBMIDI_1_cdc_notification USBMIDI_1_serialStateNotification =
    {

        USBMIDI_1_SERIAL_STATE_REQUEST_TYPE, /* bRequestType    */
        USBMIDI_1_SERIAL_STATE,              /* bNotification   */
        0u,                                         /* wValue          */
        0u,                                         /* wValueMSB       */
        0u,                                         /* wIndex          */
        0u,                                         /* wIndexMSB       */
        USBMIDI_1_SERIAL_STATE_LENGTH,       /* wLength         */
        0u,                                         /* wLengthMSB      */
        0u,                                         /* wSerialState    */
        0u,                                         /* wSerialStateMSB */
    };
    static uint8 USBMIDI_1_activeCom = 0u;
#endif /* (USBMIDI_1_ENABLE_CDC_CLASS_API != 0u) */
...
/*******************************************************************************
* Function Name: USBMIDI_1_DispatchCDCClassRqst
****************************************************************************//**
*
...
*
* \globalvars
*   USBMIDI_1_linesCoding: Contains the current line coding structure.
...
*   USBMIDI_1_linesControlBitmap: Contains the current control signal
*     bitmap. It is set by the Host using SET_CONTROL_LINE request and returned
*     to the user code by the USBFS_GetLineControl() API.
*   USBMIDI_1_linesChanged: This variable is used as a flag for the
...
uint8 USBMIDI_1_DispatchCDCClassRqst(void) 
{
    ...
    comPort = USBMIDI_1_GetInterfaceComPort((uint8)USBMIDI_1_wIndexLoReg);
    ...
    if (0u != (USBMIDI_1_bmRequestTypeReg & USBMIDI_1_RQST_DIR_D2H))
    {
        ...
        {
            case USBMIDI_1_CDC_GET_LINE_CODING:
                USBMIDI_1_currentTD.count = USBMIDI_1_LINE_CODING_SIZE;
                USBMIDI_1_currentTD.pData = USBMIDI_1_linesCoding[comPort];
            ...
            #ifdef USBMIDI_1_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK
                requestHandled = USBMIDI_1_DispatchCDCClass_CDC_READ_REQUESTS_Callback();
            #endif /* (USBMIDI_1_DISPATCH_CDC_CLASS_CDC_READ_REQUESTS_CALLBACK) */
                break;
        }
    }
    else
    {
        ...
        switch (USBMIDI_1_bRequestReg)
        {
            case USBMIDI_1_CDC_SET_LINE_CODING:
                USBMIDI_1_currentTD.count  = USBMIDI_1_LINE_CODING_SIZE;
                USBMIDI_1_currentTD.pData  = USBMIDI_1_linesCoding[comPort];
                USBMIDI_1_linesChanged[comPort] |= USBMIDI_1_LINE_CODING_CHANGED;

                requestHandled = USBMIDI_1_InitControlWrite();
                break;

            case USBMIDI_1_CDC_SET_CONTROL_LINE_STATE:
                USBMIDI_1_linesControlBitmap[comPort] = (uint8) USBMIDI_1_wValueLoReg;
                USBMIDI_1_linesChanged[comPort]      |= USBMIDI_1_LINE_CONTROL_CHANGED;
            ...
            default:
                /* Do not handle this request unless callback is defined. */
            #ifdef USBMIDI_1_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK
                requestHandled = USBMIDI_1_DispatchCDCClass_CDC_WRITE_REQUESTS_Callback();
            #endif /* (USBMIDI_1_DISPATCH_CDC_CLASS_CDC_WRITE_REQUESTS_CALLBACK) */
                break;
        }
    }
...
/***************************************************************************
* Function Name: USBMIDI_1_GetInterfaceComPort
************************************************************************//**
*   \internal
...
uint8 USBMIDI_1_GetInterfaceComPort(uint8 interface) 
{
    ...
    while (comPort < USBMIDI_1_MAX_MULTI_COM_NUM)
    {
        notEp = USBMIDI_1_cdcCommInInterruptEp[comPort];

        if (USBMIDI_1_EP[notEp].interface == interface)
        {
            res = comPort;
            comPort = USBMIDI_1_MAX_MULTI_COM_NUM;
        }
...
#if (USBMIDI_1_ENABLE_CDC_CLASS_API != 0u)
/***************************************************************************
* Function Name: USBMIDI_1_CDC_Init
************************************************************************//**
*
...
*   USBMIDI_1_SUCCESS   CDC interface was initialized correctly
*   USBMIDI_1_FAILURE   CDC interface was not initialized
*
* \globalvars
*   USBMIDI_1_linesChanged: Initialized to zero.
*   USBMIDI_1_cdcDataOutEp: Used as an OUT endpoint number.
*
* \reentrant
*  No.
*
...
uint8 USBMIDI_1_CDC_Init(void) 
{
    uint8 comPort;
    uint8 outEp;
    uint8 ret = USBMIDI_1_SUCCESS;

    USBMIDI_1_activeCom = 0u;
    USBMIDI_1_linesChanged[USBMIDI_1_COM_PORT1] = 0u;
    USBMIDI_1_linesChanged[USBMIDI_1_COM_PORT2] = 0u;

    for(comPort = 0u; comPort<USBMIDI_1_MAX_MULTI_COM_NUM; comPort++)
    {
        outEp = USBMIDI_1_cdcDataOutEp[comPort];
        if((0u != outEp) && (USBMIDI_1_MAX_EP > outEp))
        {
            USBMIDI_1_EnableOutEP(outEp);
        }

    }

    /* COM Port 1 should be correct to proceed. */
    if ((0u == USBMIDI_1_cdcDataInEp[USBMIDI_1_COM_PORT1]) \
            || (0u == USBMIDI_1_cdcDataOutEp[USBMIDI_1_COM_PORT1]) \
            || (0u ==  USBMIDI_1_cdcCommInInterruptEp[USBMIDI_1_COM_PORT1])
            || (USBMIDI_1_cdcDataInEp[USBMIDI_1_COM_PORT1] >= USBMIDI_1_MAX_EP)
            || (USBMIDI_1_cdcDataOutEp[USBMIDI_1_COM_PORT1] >= USBMIDI_1_MAX_EP)
            || (USBMIDI_1_cdcCommInInterruptEp[USBMIDI_1_COM_PORT1] >= USBMIDI_1_MAX_EP))
    {
        ret = USBMIDI_1_FAILURE;
    }
...
/*******************************************************************************
* Function Name: USBMIDI_1_PutData
****************************************************************************//**
*
*  This function sends a specified number of bytes from the location specified
*  by a pointer to the PC. The USBMIDI_1_CDCIsReady() function should be
...
*  identified. To send zero-length packet, use USBMIDI_1_PutData() API
...
* \globalvars
*
*   USBMIDI_1_cdcDataInEp: CDC IN endpoint number used for sending
*     data.
*
* \reentrant
*  No.
...
{
    uint8 epNumber = USBMIDI_1_cdcDataInEp[USBMIDI_1_activeCom];

    /* Limit length to maximum packet size for endpoint. */
    if (length > USBMIDI_1_EP[epNumber].bufferSize)
    {
        /* Caution: Data will be lost if length is greater than Max Packet Size. */
        length = USBMIDI_1_EP[epNumber].bufferSize;

        /* Halt CPU in debug mode */
        CYASSERT(0u != 0u);
    }

    USBMIDI_1_LoadInEP(epNumber, pData, length);
}


/*******************************************************************************
* Function Name: USBMIDI_1_StrLen
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USBMIDI_1_PutString
************************************************************************//**
*
...
*  The USBMIDI_1_CDCIsReady() function should be called before
*  sending data with a new call to USBMIDI_1_PutString(), to be sure
...
void USBMIDI_1_PutString(const char8 string[]) 
{
    ...
    strLength = USBMIDI_1_StrLen(string);

    do
    {
        /* Limit length to maximum packet size of endpoint. */
        sendLength = (strLength > USBMIDI_1_EP[epNumber].bufferSize) ?
                                  USBMIDI_1_EP[epNumber].bufferSize : strLength;

        /* Load IN endpoint and expose it to host. */
        USBMIDI_1_LoadInEP(epNumber, (const uint8 *)&string[bufIndex], sendLength);
        strLength -= sendLength;

        /* If more data are present to send or full packet was sent */
        if ((strLength > 0u) || (sendLength == USBMIDI_1_EP[epNumber].bufferSize))
        {
            bufIndex += sendLength;

            /* Wait until host read data from IN endpoint buffer. */
            while (USBMIDI_1_IN_BUFFER_FULL == USBMIDI_1_EP[epNumber].apiEpState)
            {
            }
            ...
            */
            if (0u == strLength)
            {
                USBMIDI_1_LoadInEP(epNumber, NULL, 0u);
            }
        }
    }
    while (strLength > 0u);
...
/***************************************************************************
* Function Name: USBMIDI_1_PutChar
************************************************************************//**
*
...
void USBMIDI_1_PutChar(char8 txDataByte) 
{
    ...
    USBMIDI_1_LoadInEP(USBMIDI_1_cdcDataInEp[USBMIDI_1_activeCom], &dataByte, 1u);
}


/*******************************************************************************
* Function Name: USBMIDI_1_PutCRLF
****************************************************************************//**
*
...
void USBMIDI_1_PutCRLF(void) 
{
    ...
    USBMIDI_1_LoadInEP(USBMIDI_1_cdcDataInEp[USBMIDI_1_activeCom], (const uint8 *)txData, 2u);
}


/*******************************************************************************
* Function Name: USBMIDI_1_GetCount
****************************************************************************//**
*
*  This function returns the number of bytes that were received from the PC.
*  The returned length value should be passed to USBMIDI_1_GetData() as
*  a parameter to read all received data. If all of the received data is not
*  read at one time by the USBMIDI_1_GetData() API, the unread data will
*  be lost.
*
* \return
...
*
* \globalvars
*   USBMIDI_1_cdcDataOutEp: CDC OUT endpoint number used.
*
*******************************************************************************/
uint16 USBMIDI_1_GetCount(void) 
{
    ...
    uint8  epNumber = USBMIDI_1_cdcDataOutEp[USBMIDI_1_activeCom];

    if (USBMIDI_1_OUT_BUFFER_FULL == USBMIDI_1_EP[epNumber].apiEpState)
    {
        bytesCount = USBMIDI_1_GetEPCount(epNumber);
    }
    else
    {
        bytesCount = 0u;
...
/*******************************************************************************
* Function Name: USBMIDI_1_DataIsReady
****************************************************************************//**
*
*  This function returns a non-zero value if the component received data or
*  received zero-length packet. The USBMIDI_1_GetAll() or
*  USBMIDI_1_GetData() API should be called to read data from the buffer
...
{
    return (USBMIDI_1_GetEPState(USBMIDI_1_cdcDataOutEp[USBMIDI_1_activeCom]));
}


/*******************************************************************************
* Function Name: USBMIDI_1_CDCIsReady
****************************************************************************//**
*
...
*  USBMIDI_1_PutData(),USBMIDI_1_PutString(),
*  USBMIDI_1_PutChar or USBMIDI_1_PutCRLF(),
...
*
* \globalvars
*   USBMIDI_1_cdcDataInEp: CDC IN endpoint number used.
*
*******************************************************************************/
uint8 USBMIDI_1_CDCIsReady(void) 
{
    return (USBMIDI_1_GetEPState(USBMIDI_1_cdcDataInEp[USBMIDI_1_activeCom]));
}


/***************************************************************************
* Function Name: USBMIDI_1_GetData
************************************************************************//**
*
...
*  The USBMIDI_1_DataIsReady() API should be called first, to be sure
*  that data is received from the host. If all received data will not be read at
*  once, the unread data will be lost. The USBMIDI_1_GetData() API should
...
uint16 USBMIDI_1_GetData(uint8* pData, uint16 length) 
{
    ...
    length = USBMIDI_1_ReadOutEP(epNumber, pData, length);

#if (USBMIDI_1_EP_MANAGEMENT_DMA_MANUAL)
    /* Wait until DMA complete transferring data from OUT endpoint buffer. */
    while (USBMIDI_1_OUT_BUFFER_FULL == USBMIDI_1_GetEPState(epNumber))
    {
    }
...
*
* \globalvars
*   - \ref USBMIDI_1_cdcDataOutEp: CDC OUT endpoint number used.
*   - \ref USBMIDI_1_EP[].bufferSize: EP max packet size is used as a
...
uint16 USBMIDI_1_GetAll(uint8* pData) 
{
    ...
    /* Read data from OUT endpoint buffer. */
    dataLength = USBMIDI_1_ReadOutEP(epNumber, pData, USBMIDI_1_EP[epNumber].bufferSize);
...
/***************************************************************************
* Function Name: USBMIDI_1_GetChar
************************************************************************//**
*
...
uint8 USBMIDI_1_GetChar(void) 
{
    ...
    (void) USBMIDI_1_ReadOutEP(epNumber, &rxData, 1u);
...
/*******************************************************************************
* Function Name: USBMIDI_1_IsLineChanged
****************************************************************************//**
*
...
*  device. The USBMIDI_1_GetDTERate(), USBMIDI_1_GetCharFormat()
*  or USBMIDI_1_GetParityType() or USBMIDI_1_GetDataBits() API
*  should be called to read data coding information.
*  The USBMIDI_1_GetLineControl() API should be called to read line
*  control information.
*
* \return
...
*
* \globalvars
*  - \ref USBMIDI_1_transferState: it is checked to be sure then OUT
*    data phase has been complete, and data written to the lineCoding or
*    Control Bitmap buffer.
*  - \ref USBMIDI_1_linesChanged: used as a flag to be aware that
...
uint8 USBMIDI_1_IsLineChanged(void) 
{
    ...
    if (USBMIDI_1_transferState == USBMIDI_1_TRANS_STATE_IDLE)
    {
        if (USBMIDI_1_linesChanged[USBMIDI_1_activeCom] != 0u)
        {
            state = USBMIDI_1_linesChanged[USBMIDI_1_activeCom];
            USBMIDI_1_linesChanged[USBMIDI_1_activeCom] = 0u;
        }
    }
...
/***************************************************************************
* Function Name: USBMIDI_1_GetDTERate
************************************************************************//**
*
...
*
* \globalvars
*  USBMIDI_1_linesCoding: First four bytes converted to uint32
...
uint32 USBMIDI_1_GetDTERate(void) 
{
    ...
    rate = USBMIDI_1_linesCoding[USBMIDI_1_activeCom][USBMIDI_1_LINE_CODING_RATE + 3u];
    rate = (rate << 8u) | USBMIDI_1_linesCoding[USBMIDI_1_activeCom][USBMIDI_1_LINE_CODING_RATE + 2u];
    rate = (rate << 8u) | USBMIDI_1_linesCoding[USBMIDI_1_activeCom][USBMIDI_1_LINE_CODING_RATE + 1u];
    rate = (rate << 8u) | USBMIDI_1_linesCoding[USBMIDI_1_activeCom][USBMIDI_1_LINE_CODING_RATE];
...
/*******************************************************************************
* Function Name: USBMIDI_1_GetCharFormat
****************************************************************************//**
*
...
*
* \globalvars
*  USBMIDI_1_linesCoding: used to get a parameter.
*
*******************************************************************************/
uint8 USBMIDI_1_GetCharFormat(void) 
{
    return (USBMIDI_1_linesCoding[USBMIDI_1_activeCom][USBMIDI_1_LINE_CODING_STOP_BITS]);
}


/*******************************************************************************
* Function Name: USBMIDI_1_GetParityType
****************************************************************************//**
*
...
{
    return (USBMIDI_1_linesCoding[USBMIDI_1_activeCom][USBMIDI_1_LINE_CODING_PARITY]);
}


/***************************************************************************
* Function Name: USBMIDI_1_GetDataBits
************************************************************************//**
*
...
{
    return (USBMIDI_1_linesCoding[USBMIDI_1_activeCom][USBMIDI_1_LINE_CODING_DATA_BITS]);
}


/***************************************************************************
* Function Name: USBMIDI_1_GetLineControl
************************************************************************//**
*
...
*
* \globalvars
*  USBMIDI_1_linesControlBitmap: used to get a parameter.
*
*******************************************************************************/
uint16 USBMIDI_1_GetLineControl(void) 
{
    return (USBMIDI_1_linesControlBitmap[USBMIDI_1_activeCom]);
}


/*******************************************************************************
* Function Name: USBMIDI_1_SendSerialState
****************************************************************************//**
*
...
*  USBMIDI_1_NotificationIsReady() API must be called to check if the
...
{
    uint8 epNumber = USBMIDI_1_cdcCommInInterruptEp[USBMIDI_1_activeCom];

    if(USBMIDI_1_SERIAL_STATE_SIZE <= USBMIDI_1_EP[epNumber].bufferSize)
    {
        /* Save current SERIAL_STATE bitmap. */
        USBMIDI_1_serialStateBitmap[USBMIDI_1_activeCom] = serialState;

        /* Add interface number */
        USBMIDI_1_serialStateNotification.wIndex = USBMIDI_1_EP[epNumber].interface;

        /*Form SERIAL_STATE data*/
        USBMIDI_1_serialStateNotification.wSerialState =    LO8(USBMIDI_1_serialStateBitmap[USBMIDI_1_activeCom]);
        USBMIDI_1_serialStateNotification.wSerialStateMSB = HI8(USBMIDI_1_serialStateBitmap[USBMIDI_1_activeCom]);

        USBMIDI_1_LoadInEP(epNumber, (uint8 *) &USBMIDI_1_serialStateNotification, sizeof(USBMIDI_1_serialStateNotification));
    }
}


/*******************************************************************************
* Function Name: USBMIDI_1_GetSerialState
****************************************************************************//**
*
...
{
    return USBMIDI_1_serialStateBitmap[USBMIDI_1_activeCom];
}


/*******************************************************************************
* Function Name: USBMIDI_1_NotificationIsReady
****************************************************************************//**
*
...
*  USBMIDI_1_SendSerialState() to ensure that any previous
...
{
    return (USBMIDI_1_EP[USBMIDI_1_cdcCommInInterruptEp[USBMIDI_1_activeCom]].apiEpState);
}


/*******************************************************************************
* Function Name: USBMIDI_1_SetComPort
****************************************************************************//**
*
...
{
    if ((USBMIDI_1_activeCom != comNumber) && \
            (comNumber < USBMIDI_1_MAX_MULTI_COM_NUM ))
    {
        USBMIDI_1_activeCom = comNumber;
    }
}


/*******************************************************************************
* Function Name: USBMIDI_1_GetComPort
****************************************************************************//**
*
...
/***************************************************************************
* Function Name: USBMIDI_1_Cdc_EpInit
************************************************************************//**
*
*  \internal
...
*   USBMIDI_1_cdcDataInEp[], USBMIDI_1_cdcCommInInterruptEp[],
*   USBMIDI_1_cdcDataOutEp[]
*
...
uint8 USBMIDI_1_Cdc_EpInit(const T_USBMIDI_1_EP_SETTINGS_BLOCK CYCODE *pEP, uint8 epNum, uint8 cdcComNums) 
{
    ...
    {
        if (epType != USBMIDI_1_EP_TYPE_INT)
        {
            USBMIDI_1_cdcDataInEp[USBMIDI_1_GET_EP_COM_NUM(cdcComNums, USBMIDI_1_CDC_IN_EP)] = epNum;
            cdcComNums |= (uint8)(USBMIDI_1_COM_PORT2 << USBMIDI_1_CDC_IN_EP);
        }
        else
        {

            USBMIDI_1_cdcCommInInterruptEp[USBMIDI_1_GET_EP_COM_NUM(cdcComNums, USBMIDI_1_CDC_NOTE_EP)] = epNum;
            cdcComNums |= (uint8)(USBMIDI_1_COM_PORT2 << USBMIDI_1_CDC_NOTE_EP);
        }
    }
    else
    {
        if (epType != USBMIDI_1_EP_TYPE_INT)
        {
            USBMIDI_1_cdcDataOutEp[USBMIDI_1_GET_EP_COM_NUM(cdcComNums, USBMIDI_1_CDC_OUT_EP)] = epNum;
            cdcComNums |= (uint8)(USBMIDI_1_COM_PORT2 << USBMIDI_1_CDC_OUT_EP);
        }
    }
    return (cdcComNums);
}
